...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const hashedPW = await axios.get(
      `http://${process.env.AUTH_ADDRESS}:${PORT}/hashed-password/${password}`,
    );
    //const hashedPW = "dummy text"
    console.log(hashedPW, email);
    res.status(201).json({ message: "User created! " + AUTH_ADDRESS });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
...
app.post("/login", async (req, res) => {
  ...
  const response = await axios.get(
    `http://${AUTH_ADDRESS}:${PORT}/token/${hashedPassword}/${password}`,
  );
